...
function verify(results){
	...
	try{
		...
		if( results[0]["_id"] !== 'denominator_PROVIDER1' || results[0].value !== 4 ){

			return {result : false, message: "denominator was "+results[0].value+" not 4 as expected"}; 

		}

		else if( results[1]["_id"] !== 'numerator_PROVIDER1' || results[1].value !== 4 ){

			return {result : false, message: "numerator was "+results[1].value+" not 4 as expected"}; 
