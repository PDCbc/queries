        ...
        if (isCurrentDrug(drugList[i])) {
            ...
            for (var j = 0; j < codes.length; j++) {
                if (codes[j].codeSystemName().toLowerCase() !== null &&
                    ...
                    emit(codes[j].code().substring(0, atcCutoff) + '_' + "whoatc" + '_' + patient.json.primary_care_provider_id, 1);
                }
                else
                {
                  emit(codes[j].code() + '_' + codes[j].codeSystemName() + '_' + patient.json.primary_care_provider_id, 1);
                }
            }
        }
    }
